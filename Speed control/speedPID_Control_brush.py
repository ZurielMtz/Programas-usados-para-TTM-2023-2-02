...
if __name__ == '__main__':
    pinBRS1 = PinMotor(23,22,21,19,36)  # Pins for the brush motor 1
    pinBRS2 = PinMotor(5,17,16,4,2)     # Pins for the brush motor 2
     # ================ CREATING MOTOR OBJETS ======================================    
    motorBRS1 = Motor(pinBRS1, 0.17007, 0.78468, 0.0092148, 537.6, 1,0,0)
    motorBRS2 = Motor(pinBRS2, 0.29756, 2.7376, 0.0080856, 537.6,1,0,0)

    while(True):
        # ---------------- Velocity for MBSH1 ------------------
        velBRS1 = motorBRS1.obtainVelocity(Tm)
        # ---------------- Velocity for MBSH2 ------------------
        velBRS2 = motorBRS2.obtainVelocity(Tm)      
        # ========= MOVINING THE MOTOR ======================
        # Move the motor in an Open Loop
        # motorBRS1.moveMotorOL(255,frequency)
        # Moves the motors in a Close Loop        
        brush_speed =  -100
        if(brush_speed != 0):
            motorBRS1.moveMotorVel(brush_speed,velBRS1,Tm)
            motorBRS2.moveMotorVel(-1*brush_speed,velBRS2,Tm)
        else:
            motorBRS1.stopMotor()
            motorBRS2.stopMotor()
        print(f'Ref1: {brush_speed} -- MBRS1: {velBRS1} -- MBRS2: {velBRS2}')
        time.sleep_ms(50)        
