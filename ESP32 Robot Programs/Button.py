...
class Button:
    mode = 'Manual'
    def __init__(self, pin, esp, msg_True, msg_False = "", peer=b"", peer2 = b"", peer3 = b"", peer4 = b""):
        # Attributes 
        ...
        # Some msg has to send it to multiple ESP32
        self.peer = peer 
        self.peer2 = peer2
        self.peer3 = peer3
        self.peer4 = peer4    
            ...
            if current_state != self.last_state and current_state ==  1: 
                ...
                if self.flag_state == True and Button.mode == 'Manual': # Se in what mode are we
                    ...
                    if self.msg_True == 'Auto':
                        Button.mode = 'Auto'
                    # Check if we need to send msg to another ESP32
                    if self.peer2 != b"":
                        self.esp.send(self.peer2, msg)
                    if self.peer3 != b"":
                        self.esp.send(self.peer3, 'Desactivate')
                    if self.peer4 != b"":
                        self.esp.send(self.peer4, msg)
                ...
                # If the mode is Auto then we stop reading the sate of the buttons, give the control to ESP32 5 and only have access to the button Stop
                    ...
                    if self.peer2 != b"":
                        self.esp.send(self.peer2, msg)
                    if self.peer3 != b'':
                        self.esp.send(self.peer3, 'Desactivar')
                    if self.peer4 != b"":
                        self.esp.send(self.peer4, msg)
